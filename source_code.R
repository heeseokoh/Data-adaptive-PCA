...
######################################################
 ...
 comp_Pseudo_before<-function(x,q){   
	...
	### Set initial estimator by ROBPCA ###
	...
	ans[[m]]<-list()
	ans[[m]][[1]]<-B
	ans[[m]][[2]]<-diag(cov(x%*% (B)))


	hatx1= x%*% B%*% t(B)
	b_tau =matrix(nrow=length(tau_range), ncol=(nrow(x)),0)
 	Z= hatx1 + com_psi( ((x)-(hatx1)), opt_w , b_tau, tau_range)  ## Empirical pseudo data Z


	# robpca<-PcaClassic(Z,q)   ## Perform conventional PCA with pseudo data Z
	# B<-t(getLoadings(robpca)[,1:q])
	# hatx=x%*% t(B)%*% B
	...
	ans[[m]]<-list()
	ans[[m]][[1]]<-B
	ans[[m]][[2]]<-diag(cov(x%*% t(B)))
	dd[m-1]<-mean( ( hatx1-hatx ) ^2 ) 
		
	while (m<10) {   ## Iterate until converged
	
		m=m+1
		ans[[m]]<-list()
 	
		robpca<-PcaClassic(Z,q)
		B<-t(getLoadings(robpca)[,1:q])
		hatx1 =x%*% t(B)%*% B
		...
		if(ncol(B)>nrow(B) ){B=t(B)}
		ans[[m]][[1]]<-B
		ans[[m]][[2]]<-diag(cov(x%*% (B)))
		dd[m-1]<-mean( ( hatx1-hatx2 ) ^2 ) 
...
comp_Pseudo<-function(x,q){   
	...
	b_tau =matrix(nrow=length(tau_range), ncol=(nrow(x)),0)
 	Z= hatx + com_psi( ((x)-(hatx)), opt_w , b_tau, tau_range)
 	#Z= hatx + (t(matrix(rep(sd(x-hatx),each=n),p,n)))*com_psi(((x)-(hatx))/(t(matrix(rep(sd(x-hatx),each=n),p,n))), opt_w , b_tau, tau_range)
  if(n>p){  	
	 BX=x%*% B
	B=lm(Z~ BX-1)$coeff	
	hatx=x%*% t(B)%*% B
 ...
 if(n<p){   
   ...
   BX=x%*% t(B)
	...
	Z= hatx2 + com_psi( ((x)-(hatx2)), opt_w , b_tau, tau_range) 
		...
		#print(mean( ( hatx1-hatx2 ) ^2 ) )
		#print(diag(cov(BX))[1:q])
 	}
